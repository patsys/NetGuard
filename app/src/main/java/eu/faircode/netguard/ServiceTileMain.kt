...
class ServiceTileMain constructor() : TileService(), OnSharedPreferenceChangeListener {
    public override fun onStartListening() {
        Log.i(ServiceTileMain.Companion.TAG, "Start listening")
    ...
    private fun update() {
        ...
        if (tile != null) {
            tile.setState(if (enabled) Tile.STATE_ACTIVE else Tile.STATE_INACTIVE)
            tile.setIcon(Icon.createWithResource(this, if (enabled) R.drawable.ic_security_white_24dp else R.drawable.ic_security_white_24dp_60))
            tile.updateTile()
        }
    }

    public override fun onStopListening() {
        Log.i(ServiceTileMain.Companion.TAG, "Stop listening")
        val prefs: SharedPreferences = PreferenceManager.getDefaultSharedPreferences(this)
        prefs.unregisterOnSharedPreferenceChangeListener(this)
    }

    public override fun onClick() {
        Log.i(ServiceTileMain.Companion.TAG, "Click")

        // Cancel set alarm
        val am: AlarmManager = getSystemService(ALARM_SERVICE) as AlarmManager
        val intent: Intent = Intent(WidgetAdmin.Companion.INTENT_ON)
        ...
        if (enabled) ServiceSinkhole.Companion.start("tile", this) else {
            ServiceSinkhole.Companion.stop("tile", this, false)

            // Auto enable
            val auto: Int = prefs.getString("auto_enable", "0")!!.toInt()
            if (auto > 0) {
                Log.i(ServiceTileMain.Companion.TAG, "Scheduling enabled after minutes=" + auto)
    ...
    companion object {
        private val TAG: String = "NetGuard.TileMain"
    }
}