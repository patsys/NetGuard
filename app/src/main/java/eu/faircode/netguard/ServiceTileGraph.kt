...
class ServiceTileGraph constructor() : TileService(), OnSharedPreferenceChangeListener {
    public override fun onStartListening() {
        Log.i(ServiceTileGraph.Companion.TAG, "Start listening")
    ...
    private fun update() {
        ...
        if (tile != null) {
            tile.setState(if (stats) Tile.STATE_ACTIVE else Tile.STATE_INACTIVE)
            tile.setIcon(Icon.createWithResource(this, if (stats) R.drawable.ic_equalizer_white_24dp else R.drawable.ic_equalizer_white_24dp_60))
            tile.updateTile()
        }
    }

    public override fun onStopListening() {
        Log.i(ServiceTileGraph.Companion.TAG, "Stop listening")
        val prefs: SharedPreferences = PreferenceManager.getDefaultSharedPreferences(this)
        prefs.unregisterOnSharedPreferenceChangeListener(this)
    }

    public override fun onClick() {
        Log.i(ServiceTileGraph.Companion.TAG, "Click")

        // Check state
        val prefs: SharedPreferences = PreferenceManager.getDefaultSharedPreferences(this)
        val stats: Boolean = !prefs.getBoolean("show_stats", false)
        if (stats && !IAB.Companion.isPurchased(ActivityPro.Companion.SKU_SPEED, this)) Toast.makeText(this, R.string.title_pro_feature, Toast.LENGTH_SHORT).show() else prefs.edit().putBoolean("show_stats", stats).apply()
        ServiceSinkhole.Companion.reloadStats("tile", this)
    }

    companion object {
        private val TAG: String = "NetGuard.TileGraph"
    }
}