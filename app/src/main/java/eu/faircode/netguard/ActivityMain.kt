...
import eu.faircode.netguard.Util.batteryOptimizing
import eu.faircode.netguard.Util.canFilter
import eu.faircode.netguard.Util.dataSaving
import eu.faircode.netguard.Util.getSelfVersionCode
import eu.faircode.netguard.Util.getSelfVersionName
import eu.faircode.netguard.Util.hasValidFingerprint
import eu.faircode.netguard.Util.hasXposed
import eu.faircode.netguard.Util.isMeteredNetwork
import eu.faircode.netguard.Util.isNetworkActive
import eu.faircode.netguard.Util.isPlayStoreInstall
import eu.faircode.netguard.Util.isPrivateDns
import eu.faircode.netguard.Util.logExtras
import eu.faircode.netguard.Util.sendLogcat
import eu.faircode.netguard.Util.setTheme
...
*/   class ActivityMain : AppCompatActivity(), OnSharedPreferenceChangeListener {
    ...
    private lateinit var dialogVpn: AlertDialog
    private lateinit var dialogDoze: AlertDialog
    private lateinit var dialogLegend: AlertDialog
    private lateinit var dialogAbout: AlertDialog
    private var iab: IAB? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        Log.i(TAG, "Create version=" + getSelfVersionName(this) + "/" + getSelfVersionCode(this))
        logExtras(intent)
        ...
        if (!intent.hasExtra(EXTRA_APPROVE)) {
            if (enabled) ServiceSinkhole.start("UI", this) else ServiceSinkhole.stop(
                "UI",
                this ,
                false
            )
        ...
        swEnabled.setOnCheckedChangeListener(CompoundButton.OnCheckedChangeListener { _, isChecked ->
            Log.i(TAG, "Switch=$isChecked")
            prefs.edit().putBoolean("enabled", isChecked).apply()
            if (isChecked) {
                val alwaysOn = Settings.Secure.getString(
                    contentResolver, "always_on_vpn_app"
                )
                ...
                if (!TextUtils.isEmpty(alwaysOn)) if (packageName == alwaysOn) {
                    if (prefs.getBoolean("filter", false)) {
                        val lockdown = Settings.Secure.getInt(
                            contentResolver, "always_on_vpn_lockdown", 0
                        )
                    ...
                    swEnabled.isChecked = false
                    Toast.makeText(this@ActivityMain, R.string.msg_always_on, Toast.LENGTH_LONG)
                        .show()
                    return@OnCheckedChangeListener
                }
                val filter = prefs.getBoolean("filter", false)
                if (filter && isPrivateDns(this@ActivityMain)) Toast.makeText(
                    this@ActivityMain,
     ...
     """.trimIndent()
        ...
        ivMetered.setOnLongClickListener {
            ...
            toast.setGravity(
                ...
                (location[1] + ivMetered.bottom - toast.view!!.paddingTop).toFloat()
                    .roundToInt()
            )
            toast.show()
            true
        ...
        if (!initialized) {
            ...
            dialogFirst = AlertDialog.Builder(this)
                ...
                .setNegativeButton(
                    R.string.app_disagree
                ) { _, _ -> if (running) finish() }
                .setOnDismissListener { }
                .create()
            dialogFirst.show()
        }
        ...
        try {
            iab = IAB(object : IAB.Delegate {
                override fun onReady(iab: IAB) {
                    try {
                        iab.updatePurchases()
                        if (!IAB.isPurchased(ActivityPro.SKU_LOG, this@ActivityMain)) prefs.edit()
                            .putBoolean("log", false).apply()
                        if (!IAB.isPurchased(ActivityPro.SKU_THEME, this@ActivityMain)) {
                            if ("teal" != prefs.getString("theme", "teal")) prefs.edit()
                                .putString("theme", "teal").apply()
                        }
                        if (!IAB.isPurchased(
                        ...
                        if (!IAB.isPurchased(ActivityPro.SKU_SPEED, this@ActivityMain)) prefs.edit()
                            .putBoolean("show_stats", false).apply()
                    } catch (ex: Throwable) {
                        Log.e(
                            TAG, """
     $ex
    ...
    override fun onNewIntent(intent: Intent) {
        ...
        if (Build.VERSION.SDK_INT < MIN_SDK || hasXposed()) return
        setIntent(intent)
        if (Build.VERSION.SDK_INT >= MIN_SDK) {
            if (intent.hasExtra(EXTRA_REFRESH)) updateApplicationList(
    ...
    override fun onResume() {
        Log.i(TAG, "Resume")
        if (Build.VERSION.SDK_INT < MIN_SDK || hasXposed()) {
            super.onResume()
            return
        }
        ...
        val llSupport = findViewById<LinearLayout>(R.id.llSupport)
        llSupport.visibility =
            if (IAB.isPurchasedAny(this) || getIntentPro(this).resolveActivity(pm) == null) View.GONE else View.VISIBLE
        super.onResume()
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        Log.i(
            TAG,
            "onActivityResult request=" + requestCode + " result=" + requestCode + " ok=" + (resultCode == RESULT_OK)
        )
        logExtras(data)
    ...
    private val onRulesChanged: BroadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            logExtras(intent)
            if (intent.hasExtra(EXTRA_CONNECTED) && intent.hasExtra(
                    EXTRA_METERED
                )
            ) {
                ivIcon.setImageResource(if (isNetworkActive(this@ActivityMain)) R.drawable.ic_security_white_24dp else R.drawable.ic_security_white_24dp_60)
                if (intent.getBooleanExtra(EXTRA_CONNECTED, false)) {
                    ...
                    ivMetered.visibility =
                        if (isMeteredNetwork(this@ActivityMain)) View.VISIBLE else View.INVISIBLE
                } else {
    ...
    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        ...
        menuSearch.setOnActionExpandListener(object : MenuItem.OnActionExpandListener {
            ...
            override fun onMenuItemActionCollapse(item: MenuItem): Boolean {
                if (intent.hasExtra(EXTRA_SEARCH) && !intent.getBooleanExtra(
                        EXTRA_RELATED,
                        false
                    )
                ) finish()
        ...
        if (!hasValidFingerprint(this) || getIntentInvite(this).resolveActivity(pm) == null) menu.removeItem(
            R.id.menu_invite
        )
    ...
    override fun onPrepareOptionsMenu(menu: Menu): Boolean {
        ...
        menu.findItem(R.id.menu_app_nointernet).isChecked =
            prefs.getBoolean("show_nointernet", true)
        menu.findItem(R.id.menu_app_disabled).isChecked = prefs.getBoolean("show_disabled", true)
        val sort = prefs.getString("sort", "name")
        if ("uid" == sort) menu.findItem(R.id.menu_sort_uid).isChecked =
            true else menu.findItem(R.id.menu_sort_name).isChecked =
            true
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        return when (item.itemId) {
            ...
            R.id.menu_log -> {
                if (canFilter()) if (IAB.isPurchased(ActivityPro.SKU_LOG, this)) startActivity(
                    Intent(
                        this,
    ...
    private fun showHints() {
        ...
        val hintWhitelist = prefs.getBoolean("hint_whitelist", true)
        llWhitelist.visibility =
            if (!(whitelist_wifi || whitelist_other) && hintWhitelist && !hintUsage) View.VISIBLE else View.GONE
    ...
    private fun checkExtras(intent: Intent) {
        ...
        if (intent.hasExtra(EXTRA_LOGCAT)) {
            ...
            if (logcat.resolveActivity(packageManager) != null) startActivityForResult(
                logcat,
                REQUEST_LOGCAT
            )
        }
    ...
    private fun checkDoze() {
        ...
        if (batteryOptimizing(this) && packageManager.resolveActivity(doze, 0) != null) {
            val prefs = PreferenceManager.getDefaultSharedPreferences(this)
    ...
    private fun checkDataSaving() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            if (dataSaving(this) && packageManager.resolveActivity(settings, 0) != null) try {
                val prefs = PreferenceManager.getDefaultSharedPreferences(this)
    ...
    private fun menu_legend() {
        ...
        view.findViewById<ImageView>(R.id.ivLockdownOn)
        view.findViewById<ImageView>(R.id.ivWifiOn)
        view.findViewById<ImageView>(R.id.ivWifiOff)
        view.findViewById<ImageView>(R.id.ivOtherOn)
        view.findViewById<ImageView>(R.id.ivOtherOff)
        view.findViewById<ImageView>(R.id.ivScreenOn)
        view.findViewById<ImageView>(R.id.ivHostAllowed)
        view.findViewById<ImageView>(R.id.ivHostBlocked)
    ...
    private fun menu_about() {
        ...
        // Show version
        tvVersionName.text = getSelfVersionName(this)
        if (!hasValidFingerprint(this)) tvVersionName.setTextColor(Color.GRAY)
        tvVersionCode.text = getSelfVersionCode(this).toString()
        ...
        // Handle rate
        btnRate.visibility = if (getIntentRate(this).resolveActivity(
                packageManager
            ) == null
    ...
    companion object {
        ...
        private fun getIntentPro(context: Context): Intent {
            return if (isPlayStoreInstall(context)) Intent(
                context,
                ActivityPro::class.java
            ) else {
     ...
     """.trimIndent()
        ...
        private fun getIntentRate(context: Context): Intent {
            ...
            if (intent.resolveActivity(context.packageManager) == null) intent = Intent(
                Intent.ACTION_VIEW,
